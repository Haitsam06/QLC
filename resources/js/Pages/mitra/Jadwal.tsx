import { useState, useMemo } from 'react';
import MitraNavbar from '../../Components/MitraNavbar';
import { CalendarDays, ChevronLeft, ChevronRight, Clock, MapPin, User, Calendar } from 'lucide-react';

/* ═══════════════════════════════════════════════════════════
   DATA MOCKUP AGENDA
═══════════════════════════════════════════════════════════ */
// Menggunakan bulan Maret 2026 sebagai contoh
const mockEvents = [
    { id: 1, date: '2026-03-12', title: 'Supervisi Guru', type: 'Kunjungan', time: '08:00', location: 'SDIT Al-Hikmah', pic: "Ust. Ainun Na'im" },
    { id: 2, date: '2026-03-19', title: 'Evaluasi Tamasya', type: 'Online', time: '09:00', location: 'Zoom Meeting', pic: 'Ust. Mushadi S.' },
    { id: 3, date: '2026-03-26', title: 'Training Manajemen', type: 'Kunjungan', time: '13:00', location: 'Aula Yayasan', pic: 'K.H. Supriyatno' },
    { id: 4, date: '2026-04-02', title: 'Rapat Kordinasi', type: 'Online', time: '10:00', location: 'Google Meet', pic: 'Tim QLC' },
    { id: 5, date: '2026-03-05', title: 'Kickoff Program', type: 'Kunjungan', time: '08:00', location: 'SDIT Al-Hikmah', pic: 'Tim QLC' },
];

const MONTH_NAMES = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
const DAYS_SHORT = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];

// Helper format YYYY-MM-DD
const formatDate = (date: Date) => {
    return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
};

/* ═══════════════════════════════════════════════════════════
   COMPONENT
═══════════════════════════════════════════════════════════ */
export default function JadwalMitra() {
    const [active, setActive] = useState('jadwal');
    const [currentDate, setCurrentDate] = useState(new Date('2026-03-09'));

    const year = currentDate.getFullYear();
    const month = currentDate.getMonth();

    // -- Navigasi Bulan --
    const nextMonth = () => setCurrentDate(new Date(year, month + 1, 1));
    const prevMonth = () => setCurrentDate(new Date(year, month - 1, 1));
    const goToday = () => setCurrentDate(new Date());

    // -- Logika Render Kalender --
    const calendarDays = useMemo(() => {
        const firstDayOfMonth = new Date(year, month, 1).getDay();
        const daysInMonth = new Date(year, month + 1, 0).getDate();
        const prevMonthDays = new Date(year, month, 0).getDate();

        const todayStr = formatDate(new Date()); // Hari ini (real)
        const days = [];

        // Padding bulan sebelumnya
        for (let i = firstDayOfMonth - 1; i >= 0; i--) {
            const day = prevMonthDays - i;
            const dStr = formatDate(new Date(year, month - 1, day));
            days.push({ day, isCurrentMonth: false, isToday: false, dateStr: dStr, events: mockEvents.filter((e) => e.date === dStr) });
        }

        // Bulan saat ini
        for (let i = 1; i <= daysInMonth; i++) {
            const dStr = formatDate(new Date(year, month, i));
            days.push({ day: i, isCurrentMonth: true, isToday: dStr === todayStr, dateStr: dStr, events: mockEvents.filter((e) => e.date === dStr) });
        }

        // Padding bulan berikutnya
        const remainingCells = 42 - days.length; // 6 baris x 7 hari = 42 sel
        for (let i = 1; i <= remainingCells; i++) {
            const dStr = formatDate(new Date(year, month + 1, i));
            days.push({ day: i, isCurrentMonth: false, isToday: false, dateStr: dStr, events: mockEvents.filter((e) => e.date === dStr) });
        }

        return days;
    }, [year, month]);

    // -- Ambil 3 Agenda Terdekat --
    const upcomingEvents = useMemo(() => {
        const todayStr = formatDate(new Date('2026-03-09')); // Simulasi hari ini
        return mockEvents
            .filter((e) => e.date >= todayStr)
            .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
            .slice(0, 3);
    }, []);

    return (
        <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
            {/* ════ TOPNAV ════ */}
            <MitraNavbar activePage={active} />

            {/* ════ PAGE WRAPPER ════ */}
            {/* PERUBAHAN: max-w-7xl dihapus, diganti w-full, dan padding pinggir disamakan dengan Navbar (px-6 lg:px-12) */}
            <div className="w-full px-6 lg:px-12 pt-8 pb-12 mx-auto">
                {/* ── Page Header ── */}
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-gray-900">Jadwal & Agenda</h1>
                    <p className="text-gray-500 mt-2">Pantau kalender kegiatan pendampingan, supervisi, dan event kerjasama bersama QLC.</p>
                </div>

                <div className="flex flex-col xl:flex-row gap-6">
                    {/* ── BAGIAN KIRI: KALENDER ── */}
                    {/* PERUBAHAN: Mengubah proporsi dari 2/3 menjadi flex-grow agar mengisi ruang kosong lebih optimal */}
                    <div className="w-full xl:flex-1 bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col overflow-hidden">
                        {/* Kalender Header */}
                        <div className="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white">
                            <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                                <Calendar className="text-green-600" size={24} />
                                {MONTH_NAMES[month]} {year}
                            </h2>
                            <div className="flex items-center gap-2 self-start sm:self-auto">
                                <button onClick={goToday} className="px-4 py-2 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors mr-2">
                                    Bulan Ini
                                </button>
                                <button onClick={prevMonth} className="p-2 text-gray-600 bg-gray-50 border border-gray-200 hover:bg-gray-100 hover:text-green-600 rounded-lg transition-colors">
                                    <ChevronLeft size={20} />
                                </button>
                                <button onClick={nextMonth} className="p-2 text-gray-600 bg-gray-50 border border-gray-200 hover:bg-gray-100 hover:text-green-600 rounded-lg transition-colors">
                                    <ChevronRight size={20} />
                                </button>
                            </div>
                        </div>

                        {/* Grid Kalender */}
                        <div className="flex-1 bg-gray-50 p-4">
                            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm h-full flex flex-col">
                                {/* Hari (Sen-Min) */}
                                <div className="grid grid-cols-7 border-b border-gray-200 bg-gray-50">
                                    {DAYS_SHORT.map((day) => (
                                        <div key={day} className="py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider border-r border-gray-200 last:border-r-0">
                                            {day}
                                        </div>
                                    ))}
                                </div>

                                {/* Tanggal */}
                                {/* Perbaikan untuk membagi tinggi cell kalender secara proporsional agar merata ke bawah layar */}
                                <div className="grid grid-cols-7 flex-1">
                                    {calendarDays.map((cell, index) => (
                                        <div
                                            key={index}
                                            className={`p-2 border-b border-gray-100 relative group transition-colors min-h-[120px]
                                            ${(index + 1) % 7 !== 0 ? 'border-r' : ''} 
                                            ${cell.isCurrentMonth ? 'bg-white hover:bg-gray-50' : 'bg-gray-50 text-gray-400'}`}
                                        >
                                            {/* Nomor Tanggal */}
                                            <div className="flex justify-end mb-2">
                                                <span
                                                    className={`flex items-center justify-center w-7 h-7 text-sm font-bold rounded-full transition-all
                                                    ${cell.isToday ? 'bg-green-600 text-white shadow-md' : cell.isCurrentMonth ? 'text-gray-700' : 'text-gray-400'}`}
                                                >
                                                    {cell.day}
                                                </span>
                                            </div>

                                            {/* Daftar Event di Hari Tersebut */}
                                            <div className="flex flex-col gap-1.5">
                                                {cell.events.map((event) => (
                                                    <div
                                                        key={event.id}
                                                        className="px-2 py-1.5 bg-green-50 border border-green-100 text-green-700 rounded text-xs font-bold leading-tight truncate cursor-pointer hover:bg-green-100 transition-colors"
                                                    >
                                                        {event.title}
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        </div>
                    </div>

                    {/* ── BAGIAN KANAN: AGENDA TERDEKAT ── */}
                    {/* PERUBAHAN: Lebar diatur tetap untuk layar besar, bukan persentase, agar kalender lebih dominan */}
                    <div className="w-full xl:w-96 shrink-0 flex flex-col gap-6">
                        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 h-full flex flex-col">
                            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                                <div className="w-10 h-10 rounded-lg bg-yellow-50 text-yellow-600 flex items-center justify-center">
                                    <Clock size={20} />
                                </div>
                                <div>
                                    <h2 className="text-lg font-bold text-gray-900">Agenda Terdekat</h2>
                                    <p className="text-xs text-gray-500 font-medium">Bulan ini</p>
                                </div>
                            </div>

                            <div className="flex flex-col gap-4 flex-1">
                                {upcomingEvents.length > 0 ? (
                                    upcomingEvents.map((evt) => (
                                        <div key={evt.id} className="p-4 bg-gray-50 border border-gray-100 rounded-xl hover:border-gray-300 transition-colors">
                                            <div className="flex justify-between items-start mb-3">
                                                <div className="text-sm font-bold text-gray-900 pr-2">{evt.title}</div>
                                                <span className="text-[10px] font-bold px-2 py-1 bg-white border border-gray-200 text-gray-600 rounded shrink-0">
                                                    {evt.date.substring(8, 10)} {MONTH_NAMES[parseInt(evt.date.substring(5, 7)) - 1].substring(0, 3)}
                                                </span>
                                            </div>

                                            <div className="space-y-2">
                                                <div className="flex items-center text-xs text-gray-500 font-medium">
                                                    <Clock size={14} className="mr-2 text-gray-400" /> {evt.time} WIB ({evt.type})
                                                </div>
                                                <div className="flex items-center text-xs text-gray-500 font-medium">
                                                    <MapPin size={14} className="mr-2 text-gray-400 shrink-0" /> <span className="truncate">{evt.location}</span>
                                                </div>
                                                <div className="flex items-center text-xs text-gray-500 font-medium">
                                                    <User size={14} className="mr-2 text-gray-400 shrink-0" /> <span className="truncate">{evt.pic}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    <div className="text-center py-10 text-sm text-gray-500">Tidak ada agenda terdekat.</div>
                                )}
                            </div>

                            <button className="w-full mt-6 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-colors">
                                Lihat Semua Riwayat
                            </button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
}
