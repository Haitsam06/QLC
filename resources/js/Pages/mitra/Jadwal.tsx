        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-200">
                <div className="px-6 pt-5 pb-4 border-b border-gray-100 flex items-start justify-between gap-4">
                    <div>
                        <span className="inline-block px-3 py-1 rounded-full text-xs font-bold bg-green-50 text-green-700 border border-green-100 mb-2">
                            {displayDate}
                        </span>
                ...
                <div className="px-6 pb-5">
                    <button onClick={onClose}
                        className="w-full py-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-sm font-bold text-gray-700 transition-colors">
                        Tutup
                    </button>
                </div>
            </div>
...
export default function JadwalMitra() {
    ...
    useEffect(() => {
        setLoading(true);
        Promise.all([
            fetch(`${BASE}/agenda?year=${year}&month=${month+1}&visibility=mitra`).then(r=>r.json()),
            fetch(`${BASE}/agenda/upcoming?visibility=mitra&limit=3`).then(r=>r.json()),
                        ...
                        <div className="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white">
                            ...
                            <div className="flex items-center gap-2 self-start sm:self-auto">
                                <button onClick={goToday}
                                    className="px-4 py-2 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors mr-2">
                                    Bulan Ini
                                </button>
                                <button onClick={prevMonth}
                                    className="p-2 text-gray-600 bg-gray-50 border border-gray-200 hover:bg-gray-100 hover:text-green-600 rounded-lg transition-colors">
                                    <ChevronLeft size={20}/>
                                </button>
                                        ...
                                        {calendarDays.map((cell, idx) => (
                                            <div key={idx}
                                                ...
                                                <div className="flex justify-end mb-2">
                                                    <span className={`flex items-center justify-center w-7 h-7 text-sm font-bold rounded-full transition-all
