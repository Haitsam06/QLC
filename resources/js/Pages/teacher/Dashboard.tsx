...
import { BookOpen, LayoutDashboard, CalendarDays, FileText, Settings, LogOut, CalendarX, CheckCircle2, AlertCircle, Plus, Star, ChevronRight, Clock, MapPin, Users, Menu, X } from 'lucide-react';
        ...
        <>
            ...
            <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
                ...
                <nav className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm h-16 flex items-center px-4 sm:px-6 lg:px-10 gap-0">
                    ...
                    <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden mr-3 p-2 text-gray-500 hover:bg-green-50 hover:text-green-700 rounded-xl transition-colors">
                        <Menu size={24} />
                    </button>
                    ...
                    <div className="hidden lg:flex items-center gap-1">
                        {navItems.map(({ icon: Icon, label, id }) => (
                            <button key={id} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all duration-200 ${activeTab === id ? 'bg-green-600 text-white shadow-md' : 'text-gray-500 hover:bg-green-50 hover:text-green-700'}`} onClick={() => setActive(id)}>
        ...
        "
                ...
                {isMobileMenuOpen && (
                    <div className="fixed inset-0 z-[100] lg:hidden">
                        ...
                        <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity" onClick={() => setIsMobileMenuOpen(false)} />
                        ...
                        <div className="absolute top-0 left-0 w-[280px] h-full bg-white shadow-2xl flex flex-col animate-slide-in-left">
                            <div className="h-16 flex items-center justify-between px-5 border-b border-gray-100">
                                ...
                                <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-gray-400 hover:bg-red-50 hover:text-red-500 rounded-lg transition-colors">
                                    <X size={20} />
                                </button>
                            </div>
                            ...
                            <div className="flex-1 p-4 flex flex-col gap-2 overflow-y-auto">
                                {navItems.map(({ icon: Icon, label, id }) => (
                                    <button key={id} className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-bold transition-all ${activeTab === id ? 'bg-green-600 text-white shadow-md' : 'text-gray-600 hover:bg-green-50 hover:text-green-700'}`} onClick={() => setActive(id)}>
                            ...
                            <div className="p-5 border-t border-gray-100 bg-gray-50 flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-green-600 flex items-center justify-center text-sm font-bold text-white shadow-sm shrink-0">{initials}</div>
                ...
                <div className="w-full px-4 sm:px-6 lg:px-10 pt-6 pb-12 mx-auto">
                    {activeTab === 'jadwal' ? (
                        ...
                        <PengaturanGuruPage profile={profile} />
                    ) : (
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="relative overflow-hidden bg-gradient-to-r from-emerald-700 to-green-900 rounded-3xl p-6 md:p-10 shadow-lg flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
                                ...
                                <div className="relative z-10 w-full">
                                    ...
                                    <h1 className="text-2xl md:text-4xl font-black text-white mb-2 tracking-tight">
                                        Ahlan wa Sahlan,
                                        <br className="sm:hidden" /> {teacherName}
                                    </h1>
                                    <p className="text-green-50 text-sm max-w-xl leading-relaxed font-medium mb-3">
                                        Semoga Allah memberkahi setiap huruf yang diajarkan hari ini. Terdapat <b className="text-white">{today_agendas.length} jadwal</b> mengajar yang menanti Anda.
                                    </p>
                                    {(profile?.email || profile?.bidang) && (
                                        <div className="flex flex-wrap gap-2 mt-1">
                                            {profile?.bidang && <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/15 border border-white/20 text-white text-xs font-bold">🎓 {profile.bidang}</span>}
                                            {profile?.email && <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/15 border border-white/20 text-white text-xs font-bold">✉ {profile.email}</span>}
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                                ...
                                <div className="bg-white rounded-3xl p-5 md:p-6 shadow-sm border border-gray-200 flex flex-col">
                                                    ...
                                                    <div className="flex flex-col gap-1 p-3 -mt-3 rounded-xl hover:bg-gray-50 transition">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span
                                                                ...
                                                                ${a.status === 'Selesai' ? 'bg-emerald-100 text-emerald-700' : a.status === 'Sedang Berjalan' ? 'bg-amber-100 text-amber-700 animate-pulse' : 'bg-gray-100 text-gray-500'}`}
                                                            >
                                                                {a.status}
                                                            </span>
                                                        </div>
                                    ...
                                    <button onClick={() => setActive('jadwal')} className="w-full mt-6 py-3 bg-green-50 border border-green-200 text-green-700 rounded-xl text-sm font-bold hover:bg-green-100 transition-colors flex items-center justify-center gap-2">
                                ...
                                <div className="xl:col-span-2 bg-white rounded-3xl p-5 md:p-6 shadow-sm border border-gray-200 flex flex-col">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-gray-100 pb-4">
                                        ...
                                        <button onClick={() => setActive('laporan')} className="px-4 py-2.5 sm:py-2 bg-green-600 text-white rounded-xl text-sm font-bold hover:bg-green-700 shadow-md flex items-center justify-center gap-2 transition-transform active:scale-95">
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center gap-3 md:w-4/12">
                                                            <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 font-bold shrink-0">{report.student_name.substring(0, 2).toUpperCase()}</div>
                                                        ...
                                                        <div className="flex flex-row gap-4 sm:gap-6 md:w-5/12 bg-gray-50/80 p-3 rounded-xl border border-gray-100 group-hover:bg-white transition-colors">
                                                            <div className="w-1/2">
                                                                ...
                                                                <div className="text-xs sm:text-sm font-bold text-gray-800 truncate" title={report.surah_or_jilid}>
                                                                    {report.surah_or_jilid}
                                                                </div>
                                                            </div>
                                                            <div className="w-1/2">
                                                                <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Ayat / Hal</div>
                                                                <div className="text-xs sm:text-sm font-bold text-gray-800 truncate" title={report.ayat_or_hal}>
                                                                    {report.ayat_or_hal}
                                                                </div>
                                                            </div>
                                                        </div>
                                    ...
                                    <div className="mt-6 pt-5 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                        ...
                                        <button onClick={() => setActive('laporan')} className="text-[13px] sm:text-sm font-bold text-green-600 hover:text-green-800 flex items-center gap-1 group self-start sm:self-auto">
